...
use crate::helpers::{generate_document_key, encrypt_document, decrypt_document, ordered_servers_keccak, decrypt_document_with_shadow, };
...
impl SecretStoreClient {
	...
	fn decrypt_key(&self, address: H160, password: Password, key: Bytes) -> Result<Vec<u8>> {
		self.accounts.decrypt(address.into(), Some(password), &DEFAULT_MAC, &key.0).map_err(|e| errors::account("Could not decrypt key.", e))
	}

	// decrypt_secret
	fn decrypt_secret(&self, address: H160, password: Password, key: Bytes) -> Result<Secret> {
		self.decrypt_key(address, password, key).and_then(|s| Secret::from_unsafe_slice(&s).map_err(|e| errors::account("invalid secret", e)))
	}
}
...
pub trait SecretStore {
	...
	fn shadow_decrypt(&self, address: H160, password: Password, decrypt_secret: H512, common_point: H512, decrypt_shadows: Vec<Bytes>, data: Bytes) -> Result<Bytes>;
...
impl SecretStore for SecretStoreClient {
	...
	fn shadow_decrypt(&self, address: H160, password: Password, decrypt_secret: H512, common_point: H512, decrypt_shadows: Vec<Bytes>, data: Bytes) -> Result<Bytes> {
		...
		decrypt_document_with_shadow(decrypt_secret.into(), common_point.into(), shadows, data.0).map(Into::into)
	}
...
mod tests {
	...
	fn sign_raw_hash_is_ok() {
		...
		let ap = AccountProvider::transient_provider();
		let account = ap.insert_account(keypair.secret().clone(), &"test".into()).unwrap();
		let secretstore_new = SecretStoreClient::new(&Arc::new(ap));
		let message = H256::random();
		let signing_response = secretstore_new.sign_raw_hash(account, "test".into(), message.clone());
	...
	fn generate_document_key_is_ok() {
		...
		let generation_response: EncryptedDocumentKey = secretstore_new.generate_document_key(account, "test".into(), H512::from_slice(&hex::decode(&skp).expect("decode failed"))).unwrap();
		assert!(secretstore_new.accounts.decrypt(
			account,
			Some("test".into()),
			&DEFAULT_MAC,
