...
fn generate_server_key(dockey_id: H256, signed_dockey_id: &str) -> Result<String> {
	...
	let path = format!("{}{}{}{}", &hex::encode(H256::as_bytes(&dockey_id)), "/", signed_dockey_id, "/1?apikey=fa05a2e1-d323-4723-96b7-4d2695a61d3f"); // TODO: work with threshhold
...
fn store_document_key(dockey_id: H256, signed_dockey_id: &str, generation_response: EncryptedDocumentKey) -> Result<String> {
	let base = Url::parse("http://localhost:8000/shadow/").unwrap();
	let path = format!("{}{}{}{}{}{}{}{}", &hex::encode(H256::as_bytes(&dockey_id)), "/", signed_dockey_id, "/", &hex::encode(H512::as_bytes(&generation_response.common_point)), "/", &hex::encode(H512::as_bytes(&generation_response.encrypted_point)), "?apikey=fa05a2e1-d323-4723-96b7-4d2695a61d3f");
	let url = base.join(&path).unwrap();
	let client = reqwest::Client::new();
	let mut buf: Vec<u8> = vec![];
	let mut res = client.post(url.as_str()).send().unwrap();
