...
fn retrieve_document_keys(dockey_id: H256, signed_dockey_id: &str) -> Result<String> {
	...
	let path = format!("{}{}{}{}", &hex::encode(H256::as_bytes(&dockey_id)), "/", signed_dockey_id, "?apikey=fa05a2e1-d323-4723-96b7-4d2695a61d3f");
...
pub fn decrypt(dockey_id: H256, ipfs_uri: String) {
    ...
    let decryption_keys: Value = serde_json::from_str(&retrieve_document_keys(dockey_id, &signed_dockey_id).unwrap()).unwrap();
    ...
    println!("{}", String::from_utf8(document).expect("file could not convert to document"));
}